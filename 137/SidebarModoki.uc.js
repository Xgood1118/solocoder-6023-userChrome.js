    ...
    try {
      CustomizableUI.createWidget({ //must run createWidget before windowListener.register because the register function needs the button added first
        ...
        onBuild: function(aDocument) {
          ...
          toolbaritem.addEventListener("command", (event) => event.target.ownerGlobal.SidebarModoki.toggle());
          return toolbaritem;
        }
      });
    }catch(e){
      if (document.getElementById("SM_Button"))
        document.getElementById('SM_Button').addEventListener("command", (event) => event.target.ownerGlobal.SidebarModoki.toggle());
    }
