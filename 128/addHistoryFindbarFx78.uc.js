...
const addHistoryFindbar78 = {
  typingSpeed: 1000,
  maxEntriesToShow:100,
  DeleteButton: false,
    
  init: function() {

    var style = `
    .findBar-history-dropmarker, .clearFindbar-button {
      & {
      ...
      &:hover {
        background-color: var(--toolbarbutton-hover-background, rgba(190,190,190,.2)) !important;
        outline: none;
      }
      &:focus-visible {
...
*/
  ...
  addDropMarker: function(findbar) {
    ...
    menupopup.setAttribute("style", "max-height:400px; max-width:30em;");
    menu.appendChild(menupopup);
    if ('bookmarksmenu_scrollbar' in window )
      bookmarksmenu_scrollbar.delayedStartup(menupopup);

    gFindBar._findField.lastInputValue = "";
  },
	// 検索バーに削除ボタン
	addClearButton: function ClearFindbarButton(findbar) {
    if (!this.DeleteButton)
      return;

		let refNode = findbar.querySelector(".findbar-find-previous");
		if (!refNode) return;
		let ClearFindbarBtn = document.createXULElement("toolbarbutton");
		refNode.parentNode.insertBefore(ClearFindbarBtn, refNode);
		ClearFindbarBtn.setAttribute("id", "clearFindbar-button");
		ClearFindbarBtn.setAttribute("class", "clearFindbar-button tabbable");
		ClearFindbarBtn.setAttribute("oncommand", "clearFindbarTweak.onCommand();");
		ClearFindbarBtn.setAttribute("label", "×");
		ClearFindbarBtn.setAttribute("tooltiptext", "Delete");
		clearFindbarTweak = {
			onCommand: function() {
				let findField = gFindBar._findField;
				findField.focus();
				findField.select();
				findField.value = "";
