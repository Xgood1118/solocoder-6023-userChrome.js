...
// @version        2022/06/12 23:00 adjust
  ...
  handleEvent: function(event) {
    switch(event.type) {
      ...
      case "resize":
        ...
        this.timer = setTimeout(() => {this.moveWindow();}, 2000);
        break;
    }
  },
  ...
  moveWindow: function() {
    ...
    if (Services.appinfo.drawInTitlebar) {
      ...
      if (window.windowState == window.STATE_NORMAL) {
        ...
        if (1.5 * this.BORDER_WIDTH >= yyb && yyb >= -1) {
          window.resizeTo(window.outerWidth, window.screen.availHeight + this.BORDER_WIDTH);
        }
        ...
        if (1.5 * this.TOP_BORDER_WIDTH >= xxr && xxr > 0) {
          window.resizeTo(window.screen.availWidth - this.BORDER_WIDTH, window.outerHeight);
        }
      }
    } else {
      if (window.windowState == window.STATE_NORMAL) {
        ...
        if (/*-this.TOP_BORDER_WIDTH*/-1 <= y && y <= /*0*/this.TOP_BORDER_WIDTH) {
          window.moveTo(x, /*-this.TOP_BORDER_WIDTH*/ -1);
        }
        y = window.screenY;
        yb = window.screenY + window.outerHeight;
        yyb = window.screen.availHeight - yb + this.BORDER_WIDTH;
        if (1.5 * this.BORDER_WIDTH >= yyb && yyb >= 0) {
          window.resizeTo(window.outerWidth,
                          window.screen.availHeight - y + this.BORDER_WIDTH -1);
        }
